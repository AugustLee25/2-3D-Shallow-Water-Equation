...
def eta_animation(X, Y, eta_list, frame_interval, filename):
    """Function to create animation of eta."""
    fig, ax = plt.subplots(1, 1)
    plt.xlabel("x [m]", fontname="serif", fontsize=12)
    plt.ylabel("y [m]", fontname="serif", fontsize=12)

    # Đảm bảo khớp lưới: Cắt dữ liệu để khớp với số ô lưới
    trimmed_eta = eta_list[0][:-1, :-1]
    ...
    def update_eta(num):
        ax.set_title(
            "Surface elevation $\eta$ after t = {:.2f} hours".format(num * frame_interval / 3600),
            fontname="serif", fontsize=16
        )
        trimmed_eta = eta_list[num][:-1, :-1]  # Cắt đúng kích thước
        pmesh.set_array(trimmed_eta.flatten())
        return pmesh,

    anim = animation.FuncAnimation(fig, update_eta, frames=len(eta_list), interval=10, blit=False)

    mpeg_writer = animation.FFMpegWriter(
        fps=60, bitrate=10000, codec="libx264",
